...
class BitcoinParser(Bank):
    ...
    def get_currency_rate(self):
        ...
        for line in result:
            if line['code'].strip().lower() in self.__currencies.keys():
                currency_rate['rate'].append(
                     {
                         'currency_id': self.__currencies.get(line['code'].lower())[0],
                         'purchase': round(float(line['rate']), 2),
                         'sale': round(float(line['rate']), 2)
                     }
                )
...
if __name__ == '__main__':
    ...
    db.close()
    parser = BitcoinParser(currencies, bank_url, bank_id)
